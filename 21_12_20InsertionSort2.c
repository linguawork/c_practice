...
void insertion_sort (int Arr[], int N)
{
    for(int i = 1; i < N; i++) // нолевой уже находится на месте
    {
        ...
        while (k > 0 && Arr[k-1] > Arr[k])// Ascending order двигаем позицию влево пока там не ноль т.е. пока позиция слева
        // имеет значение больше ноля: K > 0 т.е. пока тот элемент слева, который имеет позиицию(k-1) сущ
        {
            ...
            int tmp = Arr[k-1];// сохраняем значение левого
            Arr[k-1] = Arr[k]; // значение левого перезаписываем правым
            Arr[k] = tmp;// значение правого перезаписываем сохраненным значением левого
